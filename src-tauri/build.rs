...
fn load_app_name_from_env_file(path: PathBuf) -> Option<String> {
    ...
    for line in content.lines() {
        ...
        if let Some(value) = line.strip_prefix("VITE_APP_NAME=") {
...
fn resolve_app_name() -> String {
    if let Ok(value) = std::env::var("VITE_APP_NAME") {
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=VITE_APP_NAME");
    println!("cargo:rerun-if-changed=../.env");
    println!("cargo:rerun-if-changed=../.env.local");
    println!("cargo:rustc-env=VITE_APP_NAME={}", app_name);
    tauri_build::build()
}
